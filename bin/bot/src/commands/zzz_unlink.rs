...
use crate::db::find_zzz;
use crate::db::zzz_unlink as db_unlink;
...
pub async fn zzz_unlink(ctx: Context<'_>) -> Result<(), Error> {
    let locale = ctx.locale().unwrap_or("ja");
    ctx.defer().await?;
    let db = &ctx.data().db;
    let discord_id = ctx.author().id.get();
    let current = find_zzz(db, discord_id).await.ok().flatten();
    if current.is_none() {
